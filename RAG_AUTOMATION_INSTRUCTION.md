...
- `03_build_index.py` обновлен:
  ...
  - для ChromaDB добавлен отдельный `db_id`, основанный на `relative_path + sha256 + chunk_index`, чтобы одинаковые backup-документы не давали одинаковый Chroma ID; `chunk_id` остался прежним и продолжает использоваться для reuse embeddings cache.
