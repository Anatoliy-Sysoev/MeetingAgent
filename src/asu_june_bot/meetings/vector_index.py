...
from .vector_cache import CacheEmbeddingRequest, MeetingEmbeddingCache
...
class MeetingVectorRetriever:
    ...
    def __init__(
        ...
        self._embedding_cache = MeetingEmbeddingCache(self.cache_path)
    ...
    def score_rows(
        ...
        for row in rows:
            ...
            key = _cache_key(meeting_id, chunk_id, text_sha) if meeting_id and chunk_id else None
            keys.append(key)
            if key:
                requests.append(
        ...
        try:
            cache = self._embedding_cache.ensure_embeddings(
            ...
            for key, row in zip(keys, rows):
                if key:
                    row_vectors.append(cache.get(key))
                else:
                    vector = _l2_normalize(self.embed_fn(str(row.get("text") or "")))
                    row_vectors.append(vector)
        except Exception:  # noqa: BLE001 — cache/backend failures use lexical fallback
            return None
        return [
