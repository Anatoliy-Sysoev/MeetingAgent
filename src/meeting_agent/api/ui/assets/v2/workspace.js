...
function liveSourceName(source) {
  return source === "MIC" ? "микрофона" : "системного звука";
}
...
function liveStatusLabel(status) {
  ...
  return labels[status] || String(status || "Неизвестно");
}
...
function refinementStatusLabel(state) {
  const labels = {
    unavailable: "Нет черновика",
    draft: "Черновик",
    refining: "Уточнение",
    final: "Готово",
    failed: "Ошибка",
  };
  return labels[state] || "Недоступно";
}
...
function renderLiveRefinement(source) {
  ...
  if (state === "final") {
    ...
    summary.textContent = Number.isFinite(delta)
      ? `Канонический транскрипт готов. Разница с live-черновиком: ${delta >= 0 ? "+" : ""}${delta} символов.`
      : "Канонический offline-транскрипт готов; live-черновик сохранён.";
    button.textContent = "Повторить уточнение";
  } else if (state === "failed") {
    summary.textContent = liveReasonText(refinement.reason || "refinement_interrupted");
    button.textContent = "Продолжить уточнение";
  } else if (state === "refining") {
    summary.textContent = "Выполняется канонический offline ASR. Live-черновик доступен.";
    button.textContent = "Идёт уточнение...";
  } else if (state === "draft") {
    summary.textContent = "Черновик сохранён и исключён из индекса. Создайте канонический транскрипт.";
    button.textContent = "Уточнить offline";
  } else {
    summary.textContent = "Сначала запишите и завершите live-черновик.";
    button.textContent = "Уточнить offline";
  }
  ...
  if (!canStart) button.title = "Недостаточно прав: jobs.start";
  else if (liveIsActive(track.session)) button.title = "Перед уточнением остановите live-захват";
  else if (_activeJob !== null) button.title = "Уже выполняется другая offline-задача";
  else if (state === "final" && !force.checked) button.title = "Сначала подтвердите замену";
  else button.title = "Запустить канонический offline ASR по сохранённому live-аудио";
}
...
function renderLiveTrack(source) {
  ...
  if (session) {
    ...
    if (active) {
      readiness.textContent = session.status === "stopping"
        ? `Завершаю захват ${liveSourceName(source)}...`
        : `Идёт запись ${liveSourceName(source)}. Не закрывайте страницу.`;
    } else if (session.status === "completed") {
      readiness.textContent = "Черновик сохранён. Перед индексацией запустите offline-транскрибацию.";
    } else if (session.status === "stale") {
      readiness.textContent = "Захват остановлен после перезапуска API. Черновик может быть неполным.";
    } else {
      readiness.textContent = session.error && session.error.message
        ? session.error.message
        : "Live-захват не завершён.";
    }
  } else if (state.preflight && state.preflight.available) {
    badge.textContent = "Готово";
    badge.className = "badge ok";
    readiness.textContent = `Источник ${source} и локальная модель Vosk готовы.`;
    ...
    readiness.textContent = "Проверка готовности Live недоступна.";
  }
  ...
  partial.textContent = state.partial || (active ? "Ожидание речи..." : "Нет активного фрагмента");
