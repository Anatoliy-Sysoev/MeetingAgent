...
async function loadTranscript() {
  ...
  if (_segments.length === 0) {
    ...
    document.getElementById("seg-prev").disabled = true;
    document.getElementById("seg-next").disabled = true;
    return;
  }
...
function renderTranscriptPage() {
  const list = document.getElementById("transcript-list");
  const pageStatus = document.getElementById("seg-page-status");
  const prevButton = document.getElementById("seg-prev");
  const nextButton = document.getElementById("seg-next");
  const pageCount = Math.max(1, Math.ceil(_filteredSegmentIndices.length / TRANSCRIPT_PAGE_SIZE));
  _transcriptPage = Math.min(Math.max(_transcriptPage, 0), pageCount - 1);
  const start = _transcriptPage * TRANSCRIPT_PAGE_SIZE;
  const visibleIndices = _filteredSegmentIndices.slice(start, start + TRANSCRIPT_PAGE_SIZE);

  if (visibleIndices.length === 0) {
    list.replaceChildren(_mkEmptyMsg("Совпадений не найдено"));
  }

  const nodes = visibleIndices.map((i) => {
    const seg = _segments[i];
    const div = _mkEl("div", "seg");
    div.id = `seg-${i}`;
    div.dataset.segmentIndex = String(i);
  ...
  if (nodes.length > 0) list.replaceChildren(...nodes);
  pageStatus.textContent = _filteredSegmentIndices.length === 0
    ? "0 реплик"
...
function showTranscriptSegment(index, options = {}) {
  ...
  let filteredPosition = _filteredSegmentIndices.indexOf(index);
  if (filteredPosition < 0) {
    if (options.preserveFilter) return;
    const filter = document.getElementById("seg-filter");
    filter.value = "";
    _filteredSegmentIndices = _segments.map((_segment, segmentIndex) => segmentIndex);
    filteredPosition = index;
  }
  const targetPage = Math.floor(filteredPosition / TRANSCRIPT_PAGE_SIZE);
  if (_transcriptPage !== targetPage || !document.getElementById(`seg-${index}`)) {
    _transcriptPage = targetPage;
    renderTranscriptPage();
  }
}
...
function seekTo(sec) {
  ...
  const targetIndex = _segments.findIndex((segment) => (
    segment.start_sec != null && segment.end_sec != null && sec >= segment.start_sec && sec < segment.end_sec
  ));
  if (targetIndex >= 0) showTranscriptSegment(targetIndex);
...
function onTimeUpdate() {
  ...
  if (active >= 0) showTranscriptSegment(active, { preserveFilter: true });
  document.querySelectorAll("#transcript-list .seg").forEach((el) => {
    const index = Number(el.dataset.segmentIndex);
    const wasActive = el.classList.contains("active");
    el.classList.toggle("active", index === active);
    if (index === active && !wasActive) {
...
function filterSegments(q) {
  ...
  _filteredSegmentIndices = _segments.map((_segment, index) => index).filter((i) => {
...
function changeTranscriptPage(delta) {
  _transcriptPage += delta;
  renderTranscriptPage();
  document.getElementById("transcript-list").scrollTop = 0;
}
...
const _segPrev = document.getElementById("seg-prev");
if (_segPrev) _segPrev.addEventListener("click", () => changeTranscriptPage(-1));
const _segNext = document.getElementById("seg-next");
if (_segNext) _segNext.addEventListener("click", () => changeTranscriptPage(1));
