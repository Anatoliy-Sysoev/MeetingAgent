...
from meeting_agent.auth.deployment_safety import _deployment_mode, validate_deployment_safety
from meeting_agent.api.bootstrap_policy import (
    BOOTSTRAP_TOKEN_HEADER,
    BootstrapPolicy,
    is_local_request,
)
from meeting_agent.api.host_policy import is_local_host_header
...
def bootstrap_admin(
    ...
    policy: BootstrapPolicy = get_app_state(request).bootstrap_policy
...
def security_status(
    ...
    bootstrap_policy: BootstrapPolicy = state.bootstrap_policy
    admin_service: AdminService = _get_admin_service(request)
    return {
...
def meeting_card_diagnostics(
    ...
    service = get_app_state(request).meetings_service
