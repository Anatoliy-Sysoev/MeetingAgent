...
2. Запустить реальную offline-транскрибацию тестовой встречи через `scripts/06_transcribe_meeting.py`.
3. Добавить watcher для входящих записей из `watched_folder/`.
4. Добавить prompt-шаблоны для memo, протокола, решений, рисков, задач и классификации.
5. Собрать локальный API и небольшой UI для inbox, поиска, встреч и генерации.
