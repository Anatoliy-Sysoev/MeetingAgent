...
1. Поддерживать RAG baseline и улучшать слабые места retrieval: ПМИ, aggregation-запросы, `source_type`.
2. Проверить и улучшить offline-транскрибацию тестовой встречи через `scripts/06_transcribe_meeting.py`.
3. Проверить extractive-артефакты встречи из `scripts/07_generate_meeting_artifacts.py` и решить, где нужна ручная редактура или LLM-режим.
4. Добавить watcher для входящих записей из `watched_folder/`.
5. Добавить prompt-шаблоны классификации.
