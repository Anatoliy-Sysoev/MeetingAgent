...
def build_pivot(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for row in rows:
        ...
        issue_by_category[category][issue] += 1
    ...
    return {
        ...
        "by_review_issue": counter_dict(by_issue),
        ...
        "review_issue_by_model": {k: counter_dict(v) for k, v in sorted(issue_by_model.items())},
        ...
        "review_issue_by_category": {k: counter_dict(v) for k, v in sorted(issue_by_category.items())},
    }


def write_csv(path: Path, rows: list[dict[str, Any]]) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    fields = ["model", "category", "status", "review_verdict", "review_issue", "count"]
    grouped = Counter(
        (
            key(row, "model"),
            key(row, "category"),
            key(row, "status"),
            key(row, "review_verdict"),
            key(row, "review_issue"),
        )
        for row in rows
    )
    with path.open("w", encoding="utf-8-sig", newline="") as f:
        ...
        writer.writeheader()
        for (model, category, status, verdict, issue), count in sorted(grouped.items()):
            writer.writerow(
                {
                    "model": model,
                    "category": category,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build status/verdict/issue pivots from manual review JSONL")
