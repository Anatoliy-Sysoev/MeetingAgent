...
- `exclude_path_patterns`: glob-паттерны для точных случаев вроде `_analysis/docx_json*/**`, `**/Архив/**`, `**/Черновики и шаблоны/**`, `**/*backup*`.
...
## Процедура Настройки Exclude

Правила исключения добавляются итеративно:

1. Добавить конкретный паттерн в `exclude_path_patterns`.
2. Пересобрать RAG через `run_full_rag.ps1`.
3. Прогнать 2-3 контрольных запроса через `04_query.py --compact`.
4. Проверить, что мусор исчез, а релевантные источники не потерялись.
5. Зафиксировать изменение в `docs/decisions.md`, если правило влияет на продуктовый корпус.

Не добавлять широкий паттерн вроде `_analysis/**`, пока не проверено, что внутри нет полезных curated-источников.
