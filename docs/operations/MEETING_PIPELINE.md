...
В `/MeetingAgent` и во вкладке `Обработка` Workspace тот же параметр доступен
как `Автоматически` или точное число от 1 до 20. Выбор передается и в
одиночный запуск `diarize`, и в полный pipeline. Повторный запуск уже готовой
диаризации выполняется только явной кнопкой принудительного перезапуска.
Последний выбор и фактически найденное число кластеров сохраняются в
`meeting.json.diarization_summary` и `transcript/diarization_report.json`.
