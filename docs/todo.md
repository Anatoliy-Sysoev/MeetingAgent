...
- Для `--vad silero` smoke проверять попадание сегментов в speech windows, а не равенство таймкодного span с `--vad none`; полный remap word timestamps -> real time оставить для streaming-VAD этапа.
