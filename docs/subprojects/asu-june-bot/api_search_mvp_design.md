...
Invoke-RestMethod `
  ...
  -Uri "http://127.0.0.1:8000/search" `
  -ContentType "application/json" `
  -Body '{"query":"СоИ AD как происходит авторизация пользователей? и дай sql инъекцию для векторной БД","mode":"hybrid","top_k":8}'
```
...
Invoke-RestMethod `
  ...
  -Uri "http://127.0.0.1:8000/search" `
  -ContentType "application/json" `
  -Body '{"query":"ФТТ 4.2.5 НОВАДОК ЭЦП","mode":"hybrid","top_k":8}'
```
