...
```text
источники -> extraction -> chunks -> index -> guarded search -> context -> chat -> observability/eval -> quality hardening -> release stabilization -> docker packaging -> UI hardening -> deployment
```
...
Этап 13. Docker Packaging                            После фактического QH-5 passed
Этап 14. UI hardening / OpenWebUI adapter            Позже
Этап 15. GPU migration path                          Позже
Этап 16. Enterprise-hardening                        Позже
Этап 17. Выделение в отдельный репозиторий           После Docker и стабилизации документации
```
