...
Эта папка является runtime-артефактом и не хранится в GitHub. Для запуска на другом ПК нужно запросить готовый пакет `data/asu_june_bot_ntk` или пересобрать корпус по командам ниже.
