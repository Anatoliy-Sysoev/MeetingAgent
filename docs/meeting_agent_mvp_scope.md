...
> Status: historical planning note. All 10 tasks below are complete as of
> 2026-06. See docs/context.md and docs/todo.md for current state. Current full
> pipeline stages available via job runner: extract_audio → transcribe →
> diarize → merge → resolve_speakers → chunk → enrich → index → analyze →
> index_artifacts. Meeting-scoped search and chat are live at
> POST /meetings/{id}/search and POST /meetings/{id}/chat.
