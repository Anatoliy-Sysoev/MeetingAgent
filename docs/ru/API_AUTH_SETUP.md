...
|---|---|---|---|
| GET | `/health` | Нет | Минимальный независимый liveness (`status`, `service`, `version`) |
| GET | `/admin/diagnostics/health` | users.manage | Подробная диагностика корпуса, индекса и Ollama; только browser-сессия admin |

Публичный liveness намеренно не возвращает локальные пути, метаданные корпуса,
список моделей, счётчики или исходные ошибки зависимостей. Machine Bearer token
не даёт доступ к подробной диагностике.
