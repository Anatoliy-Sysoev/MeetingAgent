...
Project-Only Chatbot MVP начат как узкий CLI поверх текущего numpy-RAG:
...
Локальный smoke показал:

- `--sources-only --json` отрабатывает и возвращает источники из `data/numpy_index`;
- задержка возникает не на retrieval, а на генерации ответа локальной LLM;
- причина задержки: тяжелый `qwen3:8b` и слишком большой prompt при прежних значениях `top_k=12` и `max_context_chars=24000` из `config.yaml`;
- для интерактивного CLI принят быстрый профиль: `top_k=4`, `max_context_chars=6000`, `source_char_limit=1800`, `num_predict=700`, `timeout_sec=180`;
- для ручной диагностики можно запускать еще жестче: `--top-k 3 --max-context-chars 4500 --source-char-limit 1200 --num-predict 400 --timeout-sec 120`.
...
- `docs/subprojects/asu-june-bot/RUNBOOK_V2.md`: runbook v2.1 для extraction/chunking/index/search и watchdog.
...
- `scripts/09_chat.py`: prototype project-only CLI чат-бота поверх numpy-RAG.
- `monitor_asu_june_bot_index_v2.ps1`: watchdog долгого `build_index_v2 --embed-only`.
- `register_asu_june_bot_index_v2_watchdog.ps1`: регистрация scheduled task `AsuJuneBotIndexV2Watchdog`.
- `configs/prompts/project_only_chat.md`: prompt/policy для project-only ответа.
...
- Добавлен продуктовый трек `Project-Only Chatbot MVP`: чат-бот должен отвечать только по источникам проекта, показывать `sources` и отказывать без подтверждения из RAG.
- В `docs/product/PROJECT_STAGES_AND_FTT.md` добавлен `FTT-MA-21` и этап `MA-09 Project-Only Chatbot MVP`; большой UI остается позже, а чат становится первым узким пользовательским интерфейсом.
- В `docs/decisions.md` зафиксировано решение: ответ без источников считается ошибкой продукта, а отказ без источников является корректным поведением.
- Добавлен `scripts/08_process_meeting_pipeline.py`: оконный offline-pipeline `ASR -> MAP -> REDUCE -> RENDER` для готовой записи на диске.
- В `configs/schemas/meeting.schema.json` добавлен статус `processing` и поле `artifacts.pipeline_report`.
- `08_process_meeting_pipeline.py` прошел `py_compile`, `--dry-run` на тестовой встрече и 2-window smoke с resume по готовым partials.
- В 2-window smoke исправлена нормализация `source_refs`: финальные `decisions.json` и `risks.json` теперь ссылаются на реальные `segment_index` и абсолютные таймкоды из `transcript/segments.jsonl`.
- `artifacts/pipeline_report.md` фиксирует параметры запуска, время ASR/MAP/REDUCE/RENDER и статистику восстановления `source_refs`.
- Реализован первый CLI-срез Project-Only Chatbot MVP: добавлены `scripts/09_chat.py`, `configs/prompts/project_only_chat.md`, `docs/quality/project_only_chatbot_smoke_questions.md`, README-команды запуска.
- После локального smoke ускорен CLI-профиль: retrieval подтвержден через `--sources-only`, а LLM-генерация ограничена меньшим контекстом и `num_predict`.
- Для Asu June Bot v2 добавлен отдельный watchdog `AsuJuneBotIndexV2Watchdog` для долгого `scripts/asu_june_bot_build_index_v2.py --embed-only`: он проверяет `embeddings_cache_v2.jsonl`, не трогает живой процесс и перезапускает сборку только если процесс исчез до завершения cache.
...
- Для Asu June Bot: начать с модульной реализации `src/asu_june_bot/`, а не с дальнейшего усложнения `scripts/09_chat.py`.
- Для Asu June Bot: сначала реализовать `/search`, source policy и baseline, затем `/chat`.
- Для Asu June Bot v2: дождаться полного `embeddings_cache_v2`, собрать `numpy_index_v2` через `--index-only` и прогнать hybrid smoke.
...
- Следующий шаг по `FTT-MA-12`: прогнать оконный pipeline на полной тестовой встрече после выбора модели и оценить качество memo/protocol.
- Оконный offline-pipeline уже реализован как `scripts/08_process_meeting_pipeline.py`; остаются полный прогон, обработка новой записи `2026-05-08 14-32-43.mp4` и сравнение ASR с GigaAM.
...
- Прогнать `scripts/09_chat.py` локально на быстром профиле и подобрать `score_threshold` для project-only отказов на smoke-наборе.
- Следующий шаг Project-Only Chatbot: вынести CLI-логику в local API `/chat` после успешного smoke-прогона.
