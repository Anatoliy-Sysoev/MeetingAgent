...
- `RUNBOOK_V2.md` — запуск extraction/chunking/index/search/guard v2;
...
Asu June Bot v2.1/v2.2 технически собран до уровня локального Search MVP и готов к переходу на API Search MVP.
...
API `/search` должен переиспользовать тот же pipeline, что CLI `search_v2`:

```text
QueryIntent -> ProjectGuard v2 -> Retrieval -> PostReranker -> ContextBuilder -> JSON response
```

К Chat MVP переходить только после стабильного API `/search`.
...
- `scripts/asu_june_bot_guard_v2_eval.py`: guard v2 eval runner.

## Что Осталось

- Реализовать API Search MVP.
