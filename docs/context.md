...
- В backlog добавлена идея `offline-streaming pipeline`: ASR пишет transcript окнами, а MAP-этап стартует по мере готовности окон; это может сократить wall-clock время, но требует ограничения параллелизма, потому что Whisper и Ollama конкурируют за CPU.
