...
- Проверить схему `meeting.json` на первой реальной offline-встрече и создать пример папки `meetings/YYYY-MM-DD__slug/`.
- После проверки карточки встречи перейти к `FTT-MA-08`: watcher и загрузка новых записей из `watched_folder/`.
