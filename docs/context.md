...
ChromaDB больше не считается стабильным query backend: локально наблюдалась ошибка загрузки HNSW-индекса. Для критического пути поиска используется отдельный numpy backend в `data/numpy_index`.
...
- `scripts/05_build_numpy_index.py` - сборка стабильного numpy RAG backend из готовых chunks и embeddings cache.
...
- Добавлен отдельный стабильный локальный RAG backend на numpy: `scripts/rag_numpy_backend.py` и `scripts/05_build_numpy_index.py`.
