...
- Добавлен гайд `docs/quality/QUERY_FEEDBACK_LOOP.md`: процесс логирования, разметки и коррекции первых реальных запросов (active-learning поверх RAG, без дообучения весов).
- Реализовано логирование запросов в `data/query_log.jsonl`: общий `append_query_log` в `scripts/rag_common.py`, вызовы в `scripts/04_query.py` и `scripts/09_chat.py`, sensitive-запросы не пишутся, путь `paths.query_log` в `config.example.yaml`.
...
- Прогнать локально первые ~100 реальных запросов и начать ручную разметку в `data/query_log_review.jsonl` по `docs/quality/QUERY_FEEDBACK_LOOP.md`.
