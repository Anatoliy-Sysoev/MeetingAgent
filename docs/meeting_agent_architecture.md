...
Ошибки diarization исправляются без перезаписи исходных артефактов. Приватный
`transcript/speaker_overrides.json` хранит атомарный журнал событий для
стабильных `utterance_id`: прежнюю и новую техническую метку, оператора и время.
При чтении действует порядок `override -> speaker_mapping -> automatic label`;
отмена восстанавливает автоматическую атрибуцию, сохраняя событие аудита.

Пользовательский transcript строится как resolved turns. Соседние реплики
одного известного итогового спикера объединяются только при gap не более 1.5
секунды, без overlap и без смены source track. Turn сохраняет все исходные
`segment_ids/utterance_ids`, поэтому цитата и seek остаются точными.
