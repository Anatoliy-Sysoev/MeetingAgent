...
def test_eval_checks_pass_for_answered_response() -> None:
    ...
    response = ChatResponse(
        ...
        sources=[ChatSource(source_ref="S1", title="PRIVATE_SYSTEM_СоИ_AD")],
    ...
    response = ChatResponse(
        ...
        sources=[
            ChatSource(
                ...
                path="ПД НТК/Этап 1.2/8. Паспорт информационной системы/PRIVATE_SYSTEM_Паспорт ИС_v1.3.2.docx",
            )
        ],
        diagnostics={"llm_called": True},
    )
