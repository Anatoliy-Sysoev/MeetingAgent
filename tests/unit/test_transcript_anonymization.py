...
def test_anonymizer_replaces_common_sensitive_values() -> None:
    anonymizer = TranscriptAnonymizer(
        AnonymizationOptions(custom_terms={"org": ["PRIVATE_CUSTOMER"], "identifier": ["Паспорт проекта"]})
    )

    text = (
        "PRIVATE_PERSON_1 из PRIVATE_CUSTOMER отправил denis@example.com. "
        "Файл C:\\Users\\Person\\Secret\\doc.docx, ссылка https://example.test/a, код FTT-MA-08."
    )
    result = anonymizer.result_for_text(text)

    assert "PRIVATE_PERSON_1" not in result.text
    assert "PRIVATE_CUSTOMER" not in result.text
        ...
        [
            {
                ...
                "start": 0,
                "end": 1,
                "speaker_name": "PRIVATE_PERSON_1",
                "text": "PRIVATE_PERSON_1 сказал: смотри ООО «Ромашка».",
...
def test_private_mapping_is_explicit_opt_in(tmp_path: Path) -> None:
    script = _load_script()
    input_path = tmp_path / "segments.jsonl"
    _write_jsonl(input_path, [{"segment_id": "seg-1", "start": 0, "end": 1, "text": "PRIVATE_PERSON_4"}])
    out_dir = tmp_path / "out"
    ...
    assert private_map["replacements"][0]["original"] == "PRIVATE_PERSON_4"
...
def test_meeting_dir_defaults_to_transcript_anonymized(tmp_path: Path) -> None:
        ...
        json.dumps(
            {
                ...
                "speaker_mapping": {"SPEAKER_01": {"name": "PRIVATE_PERSON_3", "role": "PO"}},
            },
            ensure_ascii=False,
        ),
        encoding="utf-8",
    ...
    _write_jsonl(
        ...
        [{"segment_id": "seg-1", "start": 0, "end": 1, "speaker": "SPEAKER_01", "text": "PRIVATE_PERSON_3: задача"}],
    )
